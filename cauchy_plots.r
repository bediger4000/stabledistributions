...
hist(b, breaks=75, freq=F, angle=-45, col="orange", density=20, add=T)
hist(ab, breaks=100, freq=F, angle=30, col="blue", density=5, add=T)
dev.off()

png("cauchy_combo_pdf.png", width=750, height=600)
 hist(ab, breaks=100, freq=F, main="Combined Cauchy Distribution and Probability Density curve fit", xlab="bin center value", angle=45, col="blue", density=5, xlim=c(-5,15), ylim=c(0,0.30))
curve(dcauchy(x, location=4.96559, scale= 2.69618), add=T, col="red", lwd=1)
dev.off()
